...
obs_var = 4  # numero di variabili osservate
nl_obs_var = 4  # numero di variabili osservate con relazioni non lineari
...
model = VAE()
model = torch.load("sigm_four_nonlinear.pt").to(device)
...
plt.legend()
plt.savefig("VAE_scores_four_non_linear")
rec = []
...
loadings_fit_nl = []
for j in range(nl_obs_var):
    popt, _ = curve_fit(fit_function, latent_space, rec_train[:, -(j+1)])
    print(popt)
    loadings_fit_nl.append(popt[0])

print(loadings_fit)
print(loadings_fit_nl)
...
for row in ax:
    for col in row:
        ...
        if i>=4:
            col.scatter(latent_space, rec_train[:, i], alpha=0.5,color="red")
            col.scatter(KSI,fit_function(latent_space,loadings_fit_nl[j]),color="black",s=3)
            j+=1
        else:
            col.scatter(latent_space, rec_train[:, i], alpha=0.5,color="red")
...
plt.savefig("VAE_relation_four_non_linear")
plt.show()
